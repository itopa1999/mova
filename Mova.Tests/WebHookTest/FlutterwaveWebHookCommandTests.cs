...
public sealed class FlutterwaveWebHookCommandTests : BaseTest
{
    private const string UserPublicId = "user_webhook_test";
    private const long UserId = 42;
    private const string UserEmail = "user@mova.app";
    private const string UserFirstName = "Lucky";
    private const string TxRef = "MOVA-TX-001";
    ...
    private readonly Mock<IFlutterwaveService> _flutterwave = new();
    private readonly Mock<IIdentityService> _identityService = new();
    private readonly Mock<INotificationQueue> _notifications = new();

    private FlutterwaveWebHookCommand.Handler CreateHandler()
    {
        return new FlutterwaveWebHookCommand.Handler(
            _flutterwave.Object,
            UnitOfWork,
            _identityService.Object,
            _notifications.Object,
            Mock.Of<ILogger<FlutterwaveWebHookCommand.Handler>>());
    }
    ...
    private static byte[] BuildPayload(
        string eventType = "BANK_TRANSFER_TRANSACTION",
        string status = "successful",
        string txRef = TxRef,
        decimal amount = Amount,
        string currency = "NGN",
        long id = 123456,
        string flwRef = "FLW-REF-001")
    {
    ...
    private FlutterwaveWebHookCommand.Command CreateCommand(
        byte[]? rawBody = null,
        string? signature = ValidSignature)
    {
        return new FlutterwaveWebHookCommand.Command
        {
    ...
    {
        SetupSignatureValid(valid: true);
    ...
    [Fact]
    public async Task Handle_WithNonBankTransferEvent_ReturnsOkIgnored()
    {
        SetupSignatureValid();
        var raw = BuildPayload(eventType: "CARD_TRANSACTION");
    ...
    [Fact]
    public async Task Handle_WithNonSuccessfulStatus_ReturnsOkIgnored()
    {
        SetupSignatureValid();
    ...
    [Fact]
    public async Task Handle_WithNonSuccessfulStatus_DoesNotQueryTransaction()
    {
        SetupSignatureValid();
        var raw = BuildPayload(status: "failed");
    ...
    // =========================================================
    // 4. Payload field validation
    // =========================================================

    [Fact]
    public async Task Handle_WithMissingTxRef_ReturnsBadRequest()
    {
        SetupSignatureValid();
        var raw = BuildPayload(txRef: "");
    ...
    {
        SetupSignatureValid();
        var raw = BuildPayload(amount: 0m);
    ...
    {
        SetupSignatureValid();
        var raw = BuildPayload(amount: -100m);
    ...
    {
        SetupSignatureValid();
        var raw = BuildPayload(txRef: "NON-EXISTENT");
    ...
    public async Task Handle_WithValidWebhook_ReturnsSuccess()
    {
        ...
        Assert.Equal(TxRef, result.Data!.TxRef);
    }
    ...
    public async Task Handle_WithValidWebhook_MarksTransactionCompleted()
    {
        ...
        var saved = await Context.Transactions
            .AsNoTracking()
            .FirstAsync(x => x.Reference == TxRef);
    ...
    [Fact]
    public async Task Handle_WhenUserEmailMissing_SkipsEmailButStillSucceeds()
    {
        SetupSignatureValid();
        SetupUpdateBalance();
        SetupUserExists(email: "");
        await SeedTransactionAsync();
    ...
    [Fact]
    public async Task Handle_WhenUserNotFoundForNotification_SkipsEmailButStillSucceeds()
    {
        SetupSignatureValid();
        SetupUpdateBalance();
        _identityService
    ...
    public async Task Handle_WhenSaveChangesThrowsDbUpdateException_RollsBackAndReturnsConflict()
    {
        ...
        var handler = new FlutterwaveWebHookCommand.Handler(
            _flutterwave.Object,
            throwingUow,
            _identityService.Object,
            _notifications.Object,
